...
impl UidGenerator {
    ...
    ///
    /// Restart only heals this because UIDs are *not* persisted — see the
    /// module docs in `persist.rs`. Persisting UIDs without also persisting
    /// the counter would collide restored UIDs with freshly allocated ones.
    ///
    ...
    /// keeps this function total for every `u64`.
...
mod tests {
    ...
    /// The last usable counter value still allocates; the assert is an
