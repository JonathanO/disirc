...
- **Metrics / observability** — Structured metrics for operators: message counts (per direction), pseudoclient count, link uptime, reconnect count. Could expose via a simple HTTP endpoint or structured log events.
  - **Dropped-message counter.** Discord messages arriving while `link_phase != Ready` are silently discarded (`orchestrator.rs:383`), and nothing records it — no log line, no counter. This is *expected* behaviour (netsplit semantics), so a per-message `warn!` would be far too noisy when the IRC server is down for a sustained period. A counter is the right shape: it makes the loss visible without generating volume proportional to the outage. Note the asymmetry that makes this worth measuring — dropped *introductions* self-heal, because the burst re-emits them from `PseudoclientManager` state on `LinkUp` (`orchestrator.rs:163`), whereas dropped *messages* are lost permanently, since nothing persists or replays them. The window is reachable in normal operation: every startup and every IRC reconnect has a period where the Discord gateway is up and accepting messages while the S2S link is not yet `Ready`.
