...
3. The Gateway delivers a `GUILD_CREATE` event for each guild the bot is in. Each `GUILD_CREATE` (with both `GUILD_MEMBERS` and `GUILD_PRESENCES` intents active) includes `members` and `presences` maps. The `MemberSnapshot` emitted from this data contains **only non-offline members** — members absent from the presences map are offline and excluded. For large guilds (member count above Discord's large-guild threshold), the `GUILD_CREATE` contains online and role-bearing members only; the remaining offline members arrive in subsequent `GUILD_MEMBERS_CHUNK` events that serenity merges into the cache automatically. `disirc` does not emit a `MemberSnapshot` for `GUILD_MEMBERS_CHUNK` events. Excluded offline members are introduced lazily: when they come online via `PRESENCE_UPDATE`, or on first message (see `06-pseudoclients.md`).
