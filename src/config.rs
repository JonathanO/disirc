...
mod tests {
    ...
    /// `is_empty` is true iff all three change lists are empty.
    #[test]
    fn bridge_diff_is_empty_iff_all_lists_empty() {
        let make = |added, removed, webhook_changed| BridgeDiff {
            added,
            removed,
            webhook_changed,
        };
        let b = || vec![bridge("1", "#a", None)];
        assert!(make(vec![], vec![], vec![]).is_empty());
        assert!(!make(b(), vec![], vec![]).is_empty());
        assert!(!make(vec![], b(), vec![]).is_empty());
        assert!(!make(vec![], vec![], b()).is_empty());
    }
