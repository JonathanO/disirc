...
- [x] Run mutation testing (`cargo mutants`) and address surviving mutants
  - 1 MISSED mutant documented as equivalent: one `!queue.is_empty()` near-equivalent in connection.rs (busy-loop but identical I/O). The two `> vs >=` framing.rs mutants are gone as of 2026-07-28 — replacing the hand-rolled `\r\n` index arithmetic with `slice::strip_suffix` eliminated the mutation targets outright (23 framing.rs mutants → 9). See the `next_line` note in `.cargo/mutants.toml` for the one mutant that shifted from CAUGHT to TIMEOUT as a result.
